...
The satellite imageries were resampled using the nearest-neighbour algorithm with a spatial resolution of 30 m × 30 m for all bands, including the thermal bands.

Supervised classification was done using a random forest (RF) machine learning classifier on GEE. The RF highest classification accuracy was achieved with ten trees on bands 1, 2, 3, 4, 5, and 7 (TM and ETM+) and bands 2, 3, 4, 5, 6, and 7 (OLI) of the pre-processed images to generate the land use/land cover pattern. Since the area is well-known, geometric correction and validation were further carried out to strengthen the classification generated.
