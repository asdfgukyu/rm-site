...
The satellite imageries were resampled utilizing the nearest-neighbour algorithm with a spatial resolution of 30 m × 30 m for all bands, including the thermal bands. 


Supervised classification was done using a random forest (RF) machine learning classifier in the GEE platform. The RF highest classification accuracy was achieved with ten trees on bands 1, 2, 3, 4, 5, and 7 (TM and ETM+) and bands 2, 3, 4, 5, 6, and 7 (OLI) of the pre-processed images to generate the land use/land cover pattern. Since the area is well-known, geometric correction and validation were further carried out to strengthen the classification generated.

Five prominent land use/land cover types were identified: heavy forest, light forest, light built-up area, heavy built-up area, and water bodies.
...
Classification was conducted through random forest machines learning classifier on Google Earth Engine.

```{r echo=FALSE, out.width='90%', fig.align='center', fig.cap = '@x' }
knitr::include_graphics("img/nigeria.png")
```
